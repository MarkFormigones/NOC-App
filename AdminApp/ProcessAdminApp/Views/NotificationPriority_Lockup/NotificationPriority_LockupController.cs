...
{
    public class NotificationPriority_LockupController : BaseController
    {
        //
        // GET: /NotificationPriority_Lockup/
        ...
        public ViewResult Define(int? rId, string vw)
        {
            ...
            DAL.DataManager dataMgr = new DAL.DataManager();
            var pInfo = new DAL.NotificationPriority_Lockup();
            int adminId = -1;
            if (rId == null || rId == -1)
            {
            ...
            {
                pInfo = dataMgr.GetNotificationPriority_LockupInfoById(rId);
            }
            var pInfoM = new NotificationPriority_LockupModel(pInfo);
        ...
        [HttpPost]
        public ActionResult Define(NotificationPriority_LockupModel model)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                using (TransactionScope transaction = new TransactionScope())
                {
                    ...
                    {
                        bool isnew = true;
                        var items = new DAL.NotificationPriority_Lockup();
                        DAL.DataManager dataMgr = new DAL.DataManager();
                        if (model.Id > 0)
                        {
                            isnew = false;
                            items = dataMgr.NotificationPriority_Lockup.Where(x => x.Id == model.Id).SingleOrDefault();
                        }

                        items.Prioirity = model.Prioirity;
                        items.Description = model.Description;
                        items.Icon = model.Icon;
                        items.IconClass = model.IconClass;
                        items.EscalationInterval = model.EscalationInterval;
                        items.ReminerFrequency = model.ReminerFrequency;
                        items.Ex1 = model.Ex1;
                        items.Ex2 = model.Ex2;
                        items.LifetimeInDays = model.LifetimeInDays;
                        items.ExtensionsAllowed = model.ExtensionsAllowed;

                        if (model.FromActionType == -1) { items.IsDeleted = true; } //Delete the Object
                        items.IsActive = model.IsActive;


                        if (isnew) { dataMgr.NotificationPriority_Lockup.Add(items); }//New Object
                        dataMgr.SaveChanges();

                        ViewBag.status = "Success";
                        tempId = items.Id;
                        if (isnew)
                        { logMsg = "NotificationPriority_Lockup Added-OK"; base.Logger(DAL.General.ActivityLokup.ACTIVITY, DAL.General.OperationLokup.ADDNEW, DAL.General.ActionCommandReq.Default,logMsg, tempId, -1, -1, -1,-1,-1,-1,-1,-1); }
                        else
                        { logMsg = "NotificationPriority_Lockup Updated-OK"; base.Logger(DAL.General.ActivityLokup.ACTIVITY, DAL.General.OperationLokup.UPDATE, DAL.General.ActionCommandReq.Default,logMsg, tempId, -1, -1, -1,-1,-1,-1,-1,-1); }
            ...
            return RedirectToAction("define", "NotificationPriority_Lockup", new { rid = tempId });
        }
        public ActionResult Delete(int rid)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                using (TransactionScope transaction = new TransactionScope())
                {
                    try
                    {
                        ...
                        DAL.DataManager dataMgr = new DAL.DataManager();
                        var items = dataMgr.NotificationPriority_Lockup.Where(x => x.Id == rid).SingleOrDefault();

                        items.IsDeleted = true;  //Delete the Object  
                        items.IsActive = false;                   
                        dataMgr.SaveChanges();
                        ViewBag.status = "Deleted";
                        logMsg = "NotificationPriority_Lockup Deleted-OK";
                    ...
                    catch (Exception e)
                    {
                        ...
                        logMsg = "NotificationPriority_Lockup Deleted-ERR";
                        ModelState.AddModelError("error", e.Message);
            ...
            return RedirectToAction("NotificationPriority_LockupList", "NotificationPriority_Lockup");
        }
            ...
            if (ModelState.IsValid)
            {
                ...
                using (TransactionScope transaction = new TransactionScope())
                {
                    try
                    {
                        ...
                        dataMgr.SaveChanges();
                        logMsg = "NotificationPriority_Lockup" + msg + "-OK";
                    ...
                    {
                        ViewBag.status = "Error";
                        logMsg = "NotificationPriority_Lockup" + msg + "-ERR";
                        ModelState.AddModelError("error", e.Message);
                        base.Logger(DAL.General.ActivityLokup.EXCEPTION, DAL.General.OperationLokup.NONE, DAL.General.ActionCommandReq.Default, logMsg, rid, -1, -1, -1, -1, -1, -1, -1, -1);
        ...
        public ViewResult NotificationPriority_LockupList()
        {
            DAL.DataManager dataMgr = new DAL.DataManager();
            var items = dataMgr.GetNotificationPriority_Lockup_List();
            var Roleitems = dataMgr.GetUser_Roles();
            var ls_items = new List<Models.Definitions.NotificationPriority_LockupModel>();
            foreach (var item in items)
            {
                var l_item = new Models.Definitions.NotificationPriority_LockupModel();
                l_item.Id = item.Id;
                l_item.Prioirity = item.Prioirity;
                l_item.Description = item.Description;
                l_item.Icon = item.Icon;
                l_item.IconClass = item.IconClass;
                l_item.EscalationInterval = item.EscalationInterval;
                l_item.ReminerFrequency = item.ReminerFrequency;
                l_item.Ex1 = item.Ex1;
                l_item.Ex2 = item.Ex2;
                l_item.LifetimeInDays = item.LifetimeInDays;
                l_item.ExtensionsAllowed = item.ExtensionsAllowed;
