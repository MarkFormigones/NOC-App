...
{
    public class OperationLockupController : BaseController
    {
        //
        // GET: /AppLockup/
        ...
        public ViewResult Define(int? rId, string vw)
        {
            ...
            DAL.DataManager dataMgr = new DAL.DataManager();
            var pInfo = new DAL.OperationLockup();
            int adminId = -1;
            if (rId == null || rId == -1)
            {
            ...
            {
                pInfo = dataMgr.GetOperationLockupInfoById(rId);
            }


            var pInfoM = new OperationLockupModel(pInfo);
        ...
        [HttpPost]
        public ActionResult Define(OperationLockupModel model)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                using (TransactionScope transaction = new TransactionScope())
                {
                    ...
                    {
                        bool isnew = true;
                        var items = new DAL.OperationLockup();
                        DAL.DataManager dataMgr = new DAL.DataManager();
                        if (model.Id > 0)
                        {
                            isnew = false;
                            items = dataMgr.OperationLockups.Where(x => x.Id == model.Id).SingleOrDefault();
                        }
                        ...
                        if (isnew) { items.IsDeleted = false; dataMgr.OperationLockups.Add(items); }//New Object
                        dataMgr.SaveChanges();

                        ViewBag.status = "Success";
                        tempId = items.Id;
                        if (isnew)
                        { logMsg = "OperationLockup Added-OK"; base.Logger(DAL.General.ActivityLokup.ACTIVITY, DAL.General.OperationLokup.ADDNEW, DAL.General.ActionCommandReq.Default,logMsg, tempId, -1, -1, -1,-1,-1,-1,-1,-1); }
                        else
                        { logMsg = "OperationLockup Updated-OK"; base.Logger(DAL.General.ActivityLokup.ACTIVITY, DAL.General.OperationLokup.UPDATE, DAL.General.ActionCommandReq.Default,logMsg, tempId, -1, -1, -1,-1,-1,-1,-1,-1); }
            ...
            return RedirectToAction("define", "OperationLockup", new { rid = tempId });
        }
        public ActionResult Delete(int rid)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                using (TransactionScope transaction = new TransactionScope())
                {
                    try
                    {
                        ...
                        DAL.DataManager dataMgr = new DAL.DataManager();
                        var items = dataMgr.OperationLockups.Where(x => x.Id == rid).SingleOrDefault();

                        items.IsDeleted = true;  //Delete the Object   
                        items.IsActive = false;                  
                        dataMgr.SaveChanges();
                        ViewBag.status = "Deleted";
                        logMsg = "OperationLockup Deleted-OK";
                    ...
                    catch (Exception e)
                    {
                        ...
                        logMsg = "OperationLockup Deleted-ERR";
                        ModelState.AddModelError("error", e.Message);
            ...
            return RedirectToAction("OperationLockupList", "OperationLockup");
        }
        ...
        public ActionResult Deactivate(int rid, Boolean val)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                using (TransactionScope transaction = new TransactionScope())
                {
                    ...
                    {
                        ViewBag.status = "Error";
                        logMsg = "OperationLockup" + msg + "-ERR";
                        ModelState.AddModelError("error", e.Message);
                        base.Logger(DAL.General.ActivityLokup.EXCEPTION, DAL.General.OperationLokup.NONE, DAL.General.ActionCommandReq.Default, logMsg, rid, -1, -1, -1, -1, -1, -1, -1, -1);
        ...
        public ViewResult OperationLockupList()
        {
            DAL.DataManager dataMgr = new DAL.DataManager();
            var items = dataMgr.GetOperationLockup_List();
            var Roleitems = dataMgr.GetUser_Roles();
            var ls_items = new List<Models.Definitions.OperationLockupModel>();
            foreach (var item in items)
            {
                var l_item = new Models.Definitions.OperationLockupModel();
                l_item.Id = item.Id;                              
