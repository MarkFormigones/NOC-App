...
{
    public class NotificationLockupController : BaseController
    {
        //
        // GET: /AppLockup/
        ...
        public ViewResult Define(int? rId, string vw)
        {
            ...
            DAL.DataManager dataMgr = new DAL.DataManager();
            var pInfo = new DAL.NotificationLockup();
            int adminId = -1;
            if (rId == null || rId == -1)
            {
            ...
            {
                pInfo = dataMgr.GetNotificationLockupInfoById(rId);
            }


            var pInfoM = new NotificationLockupModel(pInfo);
        ...
        [HttpPost]
        public ActionResult Define(NotificationLockupModel model)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                using (TransactionScope transaction = new TransactionScope())
                {
                    ...
                    {
                        bool isnew = true;
                        var items = new DAL.NotificationLockup();
                        DAL.DataManager dataMgr = new DAL.DataManager();
                        if (model.Id > 0)
                        {
                            isnew = false;
                            items = dataMgr.NotificationLockups.Where(x => x.Id == model.Id).SingleOrDefault();
                        }
                        items.Name = model.Name;
                        items.Icon = model.Icon;
                        items.IconClass = model.IconClass;
                        items.Tag = model.Tag;
                        items.TypeId = model.TypeId;                        
                        items.Dated = model.Dated;
                        items.IsEscalatable = model.IsEscalatable;
                        items.IsMentionDueDate = model.IsMentionDueDate;
                        items.RedirectController = model.RedirectController;
                        items.RedirectAction = model.RedirectAction;
                        items.RedirectExtraParam = model.RedirectExtraParam;
                        items.RedirectExParam1 = model.RedirectExParam1;
                        items.RedirectExParam2 = model.RedirectExParam2;
                        items.RedirectExParam3 = model.RedirectExParam3;
                        items.RedirectExParamDefault = model.RedirectExParamDefault;
                        

                        if (model.FromActionType == -1) { items.IsDeleted = true; } //Delete the Object
                        items.IsActive = model.IsActive;

                        if (isnew) { items.IsDeleted = false; dataMgr.NotificationLockups.Add(items); }//New Object
                        dataMgr.SaveChanges();

                        ViewBag.status = "Success";
                        tempId = items.Id;
                        if (isnew)
                        { logMsg = "NotificationLockup Added-OK"; base.Logger(DAL.General.ActivityLokup.ACTIVITY, DAL.General.OperationLokup.ADDNEW, DAL.General.ActionCommandReq.Default,logMsg, tempId, -1, -1, -1,-1,-1,-1,-1,-1); }
                        else
                        { logMsg = "NotificationLockup Updated-OK"; base.Logger(DAL.General.ActivityLokup.ACTIVITY, DAL.General.OperationLokup.UPDATE, DAL.General.ActionCommandReq.Default,logMsg, tempId, -1, -1, -1,-1,-1,-1,-1,-1); }
            ...
            return RedirectToAction("define", "NotificationLockup", new { rid = tempId });
        }
        public ActionResult Delete(int rid)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                using (TransactionScope transaction = new TransactionScope())
                {
                    try
                    {
                        ...
                        DAL.DataManager dataMgr = new DAL.DataManager();
                        var items = dataMgr.NotificationLockups.Where(x => x.Id == rid).SingleOrDefault();

                        items.IsDeleted = true;  //Delete the Object   
                        items.IsActive = false;                  
                        dataMgr.SaveChanges();
                        ViewBag.status = "Deleted";
                        logMsg = "NotificationLockup Deleted-OK";
                    ...
                    catch (Exception e)
                    {
                        ...
                        logMsg = "NotificationLockup Deleted-ERR";
                        ModelState.AddModelError("error", e.Message);
            ...
            return RedirectToAction("NotificationLockupList", "NotificationLockup");
        }
            ...
            if (ModelState.IsValid)
            {
                ...
                using (TransactionScope transaction = new TransactionScope())
                {
                    try
                    {
                        ...
                        dataMgr.SaveChanges();
                        logMsg = "NotificationLockup" + msg + "-OK";
                    ...
                    {
                        ViewBag.status = "Error";
                        logMsg = "NotificationLockup" + msg + "-ERR";
                        ModelState.AddModelError("error", e.Message);
                        base.Logger(DAL.General.ActivityLokup.EXCEPTION, DAL.General.OperationLokup.NONE, DAL.General.ActionCommandReq.Default, logMsg, rid, -1, -1, -1, -1, -1, -1, -1, -1);
        ...
        public ViewResult NotificationLockupList()
        {
            DAL.DataManager dataMgr = new DAL.DataManager();
            var items = dataMgr.GetNotificationLockup_List();
            var Roleitems = dataMgr.GetUser_Roles();
            var ls_items = new List<Models.Definitions.NotificationLockupModel>();
            foreach (var item in items)
            {
                var l_item = new Models.Definitions.NotificationLockupModel();
                l_item.Id = item.Id;
                ...
                l_item.IsEscalatable = item.IsEscalatable;
                l_item.IsMentionDueDate = item.IsMentionDueDate;
                l_item.RedirectController = item.RedirectController;
                l_item.RedirectAction = item.RedirectAction;
                l_item.RedirectExtraParam = item.RedirectExtraParam;
                l_item.RedirectExParam1 = item.RedirectExParam1;
                l_item.RedirectExParam2 = item.RedirectExParam2;
                l_item.RedirectExParam3 = item.RedirectExParam3;
                l_item.RedirectExParamDefault = item.RedirectExParamDefault;
