...
{
    public class CommandLockupController : BaseController
    {
        //
        // GET: /AppLockup/
        ...
        public ViewResult Define(int? rId, string vw)
        {
            ...
            DAL.DataManager dataMgr = new DAL.DataManager();
            var pInfo = new DAL.CommandLockup();
            int adminId = -1;
            if (rId == null || rId == -1)
            {
            ...
            {
                pInfo = dataMgr.GetCommandLockupInfoById(rId);
            }


            var pInfoM = new CommandLockupModel(pInfo);
        ...
        [HttpPost]
        public ActionResult Define(CommandLockupModel model)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                using (TransactionScope transaction = new TransactionScope())
                {
                    ...
                    {
                        bool isnew = true;
                        var items = new DAL.CommandLockup();
                        DAL.DataManager dataMgr = new DAL.DataManager();
                        if (model.Id > 0)
                        {
                            isnew = false;
                            items = dataMgr.CommandLockups.Where(x => x.Id == model.Id).SingleOrDefault();
                        }

                        items.Command = model.Command;
                        items.value = model.value;
                        items.Comment = model.Comment;
                        items.ComamandIcon = model.ComamandIcon;                       
                        items.CommandClass = model.CommandClass;
                        

                        if (model.FromActionType == -1) { items.IsDeleted = true; } //Delete the Object
                        items.IsActive = model.IsActive;

                        if (isnew) { items.IsDeleted = false; dataMgr.CommandLockups.Add(items); }//New Object
                        dataMgr.SaveChanges();

                        ViewBag.status = "Success";
                        tempId = items.Id;
                        if (isnew)
                        { logMsg = "CommandLockup Added-OK"; base.Logger(DAL.General.ActivityLokup.ACTIVITY, DAL.General.OperationLokup.ADDNEW, DAL.General.ActionCommandReq.Default,logMsg, tempId, -1, -1, -1,-1,-1,-1,-1,-1); }
                        else
                        { logMsg = "CommandLockup Updated-OK"; base.Logger(DAL.General.ActivityLokup.ACTIVITY, DAL.General.OperationLokup.UPDATE, DAL.General.ActionCommandReq.Default,logMsg, tempId, -1, -1, -1,-1,-1,-1,-1,-1); }
            ...
            return RedirectToAction("define", "CommandLockup", new { rid = tempId });
        }
        public ActionResult Delete(int rid)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                using (TransactionScope transaction = new TransactionScope())
                {
                    try
                    {
                        ...
                        DAL.DataManager dataMgr = new DAL.DataManager();
                        var items = dataMgr.CommandLockups.Where(x => x.Id == rid).SingleOrDefault();

                        items.IsDeleted = true;  //Delete the Object   
                        items.IsActive = false;                  
                        dataMgr.SaveChanges();
                        ViewBag.status = "Deleted";
                        logMsg = "CommandLockup Deleted-OK";
                    ...
                    catch (Exception e)
                    {
                        ...
                        logMsg = "CommandLockup Deleted-ERR";
                        ModelState.AddModelError("error", e.Message);
            ...
            return RedirectToAction("CommandLockupList", "CommandLockup");
        }
        ...
        public ViewResult CommandLockupList()
        {
            DAL.DataManager dataMgr = new DAL.DataManager();
            var items = dataMgr.GetCommandLockup_List();
            var Roleitems = dataMgr.GetUser_Roles();
            var ls_items = new List<Models.Definitions.CommandLockupModel>();
            foreach (var item in items)
            {
                var l_item = new Models.Definitions.CommandLockupModel();
                l_item.Id = item.Id;
                l_item.Command = item.Command;
                l_item.value = item.value;
                l_item.Comment = item.Comment;
                l_item.ComamandIcon = item.ComamandIcon;
                l_item.CommandClass = item.CommandClass;               
